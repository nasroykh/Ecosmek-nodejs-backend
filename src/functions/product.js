 ...
 */
const _create_new_product = async (name, brand, price, category, img, type, description) => {

    try {

//      We create a new product using information received
        const product = new Product({name, brand, price, category, img, type, description})
...
//              If the wordskey match any product attributes
                if (products[i][key].toString().toLowerCase().includes(wordsKey)){

//                  Add the wanted product in {Array: specificProducts}
                    specificProducts.push(products[i])
                    
//                  Stop checking in this product because it has been pushed to the {Array: specificProducts}
                    break;
...
//      Get the wanted products 
        const specificProducts = await Product.findOne({[key]: [value].toLowerCase()})
