...
router.get('/find_products_by_key', async function (req, res, next) {
    const products = await ProductsF._get_products_by_key(req.body.key, req.body.value)
    res.send(products)
})

/**   Find products by words   **/
router.get('/find_products_by_words', async function (req, res, next) {
    try{
...
router.get('/products', async function (req, res, next) {
    ...
    console.log(products[1].img);
    res.set('Content-Type', 'image/png')
    res.sendFile(products[1].img)
})
/*-----------------   MANAGER   -----------------*/
...
router.post('/create_new_product', [authentication.auth, authentication._admin_auth], upload.single('productPicture'), async function (req, res, next) {
    try {
        const creationStatue = await ProductsF._create_new_product(req.body.name, req.body.brand, req.body.price, req.body.category, req.file.buffer, req.body.type)
